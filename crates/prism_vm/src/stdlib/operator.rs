...
impl Module for OperatorModule {
    ...
    fn get_attr(&self, name: &str) -> ModuleResult {
        match name {
            ...
            "__truth__" => Ok(builtin_value(&TRUTH_FUNCTION)),
            "not_" | "__not__" => Ok(builtin_value(&NOT_FUNCTION)),
            "contains" | "__contains__" => Ok(builtin_value(&CONTAINS_FUNCTION)),
            "countOf" => Ok(builtin_value(&COUNT_OF_FUNCTION)),
            "index" | "__index__" => Ok(builtin_value(&INDEX_FUNCTION)),
            "indexOf" => Ok(builtin_value(&INDEX_OF_FUNCTION)),
            "length_hint" => Ok(builtin_value(&LENGTH_HINT_FUNCTION)),
            "eq" | "__eq__" => Ok(builtin_value(&EQ_FUNCTION)),
            "ne" | "__ne__" => Ok(builtin_value(&NE_FUNCTION)),
            "lt" | "__lt__" => Ok(builtin_value(&LT_FUNCTION)),
            "le" | "__le__" => Ok(builtin_value(&LE_FUNCTION)),
            "gt" | "__gt__" => Ok(builtin_value(&GT_FUNCTION)),
            "ge" | "__ge__" => Ok(builtin_value(&GE_FUNCTION)),
...
fn operator_or(vm: &mut VirtualMachine, args: &[Value]) -> Result<Value, BuiltinError> {
    expect_arg_count("or_", args, 2)?;
    bitwise_or_value(vm, args[0], args[1]).map_err(runtime_error_to_builtin_error)
}
