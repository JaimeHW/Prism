...
mod tests {
    ...
    fn test_base64_round_trips_and_respects_newline_keyword() {
        let encoded =
            b2a_base64_builtin(&[test_bytes(b"hello")], &[("newline", Value::bool(false))])
        ...
        let with_newline =
            b2a_base64_builtin(&[test_bytes(b"hello")], &[]).expect("newline default");
    ...
    fn test_hexlify_and_unhexlify() {
        let encoded = b2a_hex_builtin(&[test_bytes(&[0xb9, 0x01, 0xef])], &[])
            .expect("hexlify should succeed");
    ...
    fn test_crc_functions_are_incremental() {
        ...
        let second =
            crc32_builtin(&[test_bytes(b" this string."), first]).expect("crc32 second chunk");
        ...
        let hqx_first =
            crc_hqx_builtin(&[test_bytes(b"Test the CRC-32 of"), Value::int(0).unwrap()])
                .expect("crc_hqx first chunk");
        let hqx_second = crc_hqx_builtin(&[test_bytes(b" this string."), hqx_first])
            .expect("crc_hqx second chunk");
