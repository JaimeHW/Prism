...
impl CompiledBytesPattern {
    ...
    fn match_(&self, text: &[u8]) -> Option<BytesMatch> {
        self.regex.captures(text).and_then(|captures| {
            ...
            if matched.start() == 0 {
                Some(BytesMatch::from_captures_with_regex(
                    &captures,
                    text,
                    &self.regex,
                ))
    ...
    fn search(&self, text: &[u8]) -> Option<BytesMatch> {
        self.regex
            .captures(text)
            .map(|captures| BytesMatch::from_captures_with_regex(&captures, text, &self.regex))
    }
    ...
    fn sub_n(&self, repl: &[u8], text: &[u8], count: usize) -> Vec<u8> {
        if count == 0 {
            self.regex.replace_all(text, repl).into_owned()
        } else if count == 1 {
            self.regex.replace(text, repl).into_owned()
        } else {
            let mut result = text.to_vec();
            let mut replaced = 0;
            while replaced < count {
                let new = self.regex.replace(&result, repl).into_owned();
                if new == result {
                    break;
                }
                result = new;
    ...
    fn subn(&self, repl: &[u8], text: &[u8], count: usize) -> (Vec<u8>, usize) {
        let replacements = if count == 0 {
            ...
            self.regex.find_iter(text).count().min(count)
        };
    ...
    fn split_n(&self, text: &[u8], maxsplit: usize) -> Vec<Vec<u8>> {
        if maxsplit == 0 {
            self.regex.split(text).map(|part| part.to_vec()).collect()
        } else {
            self.regex
