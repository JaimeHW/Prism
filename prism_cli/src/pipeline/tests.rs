...
fn test_run_module_supports_relative_imports() {
    ...
    let config = RuntimeConfig::from_args(&crate::args::PrismArgs::default());
    let code = run_module_with_search_paths("pkg.main", &config, std::slice::from_ref(&temp.path));
    assert_eq!(code, ExitCode::from(0));
}
...
fn test_run_module_supports_parent_relative_imports() {
    ...
    let config = RuntimeConfig::from_args(&crate::args::PrismArgs::default());
    let code =
        run_module_with_search_paths("pkg.subpkg.main", &config, std::slice::from_ref(&temp.path));
    assert_eq!(code, ExitCode::from(0));
}
