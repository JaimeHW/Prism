...
pub fn run_file(path: &Path, _config: &RuntimeConfig) -> ExitCode {
...
fn execute_source(source: &str, filename: &str) -> ExitCode {
    // Phase 1: Parse.
...
mod tests {
    ...
    fn test_execute_empty_source() {
        let code = execute_source("", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }

    #[test]
    fn test_execute_simple_assignment() {
        let code = execute_source("x = 42", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }

    #[test]
    fn test_execute_multiple_statements() {
        let code = execute_source("x = 1\ny = 2\nz = x + y", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }
    ...
    fn test_execute_while_loop() {
        let code = execute_source("x = 0\nwhile x < 5:\n    x = x + 1", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }

    #[test]
    fn test_execute_list_operations() {
        let code = execute_source("lst = [1, 2, 3]\nx = len(lst)", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }

    #[test]
    fn test_execute_dict_operations() {
        let code = execute_source("d = {'a': 1, 'b': 2}", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }
    ...
    fn test_execute_string_operations() {
        let code = execute_source("s = 'hello'\nx = len(s)", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }

    #[test]
    fn test_execute_print_call() {
        let code = execute_source("print('hello, world')", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }
    ...
    fn test_execute_class_definition() {
        let code = execute_source("class Foo:\n    x = 42", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }

    #[test]
    fn test_execute_try_except() {
        let code = execute_source("try:\n    x = 1\nexcept:\n    x = 0", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }
    ...
    fn test_execute_list_comprehension() {
        let code = execute_source("squares = [x * x for x in range(5)]", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }
    ...
    fn test_execute_syntax_error_returns_error_code() {
        let code = execute_source("def", "<test>");
        assert_ne!(code, ExitCode::from(0));
    }

    #[test]
    fn test_execute_name_error_returns_error_code() {
        let code = execute_source("print(undefined_variable)", "<test>");
        assert_ne!(code, ExitCode::from(0));
    }
    ...
    fn test_execute_only_comments() {
        let code = execute_source("# just a comment\n# another comment", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }
    ...
    fn test_execute_pass_statement() {
        let code = execute_source("pass", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }
    ...
    fn test_execute_comparison_chain() {
        let code = execute_source("result = 1 < 2 < 3", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }
    ...
    fn test_execute_tuple_unpacking() {
        let code = execute_source("a, b = 1, 2", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }
    ...
    fn test_execute_lambda() {
        let code = execute_source("f = lambda x: x * 2\nresult = f(21)", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }

    #[test]
    fn test_execute_assert_passes() {
        let code = execute_source("assert True", "<test>");
        assert_eq!(code, ExitCode::from(0));
    }
    ...
    fn test_execute_assert_fails_returns_error() {
        let code = execute_source("assert False", "<test>");
        assert_ne!(code, ExitCode::from(0));
    }
